...
public class PlasmidsGenerator {
    ...
    public void flattenPlasmidDesigns(Path sbolFile, Path outFile) throws SBOLValidationException, IOException, SBOLConversionException, URISyntaxException {
        // Path flattenDir = outDir.resolve("flattened");

        SBOLDocument doc = SBOLReader.read(sbolFile.toFile());
        List<ComponentDefinition> flatCmpDefs = new ArrayList();

        for (ComponentDefinition cmpDef : doc.getComponentDefinitions()) {
            ComponentDefinition flatCmpDef = null;
            try {
                flatCmpDef = flattenPlasmidDesign(cmpDef, doc);
            } catch (SBOLValidationException e) {
                System.out.printf("Unable to flatten %s component definition%n", cmpDef.getDisplayId());
            }

            if (flatCmpDef != null) {
                flatCmpDefs.add(flatCmpDef);
            }
        }

        // Path outFile = flattenDir.resolve(sbolFile.getFileName());
        SBOLWriter.write(doc, outFile.toFile());
    }

    protected ComponentDefinition flattenPlasmidDesign(ComponentDefinition plasmid, SBOLDocument doc) throws SBOLValidationException, URISyntaxException {
        String name = plasmid.getName();
        String displayId = plasmid.getDisplayId();

        System.out.println("Flattening ....");
        ComponentDefinition flattenPlasmid = transformer.flattenSequences(plasmid, displayId + "_flat", doc);
        ...
        return flattenPlasmid;
    }

    public void addPlasmidAnnotations(Path sbolFile, Path outFile, String description, String notes) throws SBOLValidationException, IOException, SBOLConversionException, URISyntaxException {
        // Path flattenDir = outDir.resolve("flattened");

        SBOLDocument doc = SBOLReader.read(sbolFile.toFile());
        List<ComponentDefinition> annoCmpDefs = new ArrayList();

        for (ComponentDefinition cmpDef : doc.getComponentDefinitions()) {
            ComponentDefinition annoCmpDef = null;
            try {
                annoCmpDef = addPlasmidAnnotations(cmpDef, description, notes);
            } catch (SBOLValidationException e) {
                System.out.printf("Unable to flatten %s component definition%n", cmpDef.getDisplayId());
            }

            if (annoCmpDef != null) {
                annoCmpDefs.add(annoCmpDef);
            }
        }
    ...
    protected ComponentDefinition addPlasmidAnnotations(ComponentDefinition plasmid, String description, String notes) throws SBOLValidationException {
        if(description != null && !description.isEmpty()) {
            Annotation origAnno = plasmid.getAnnotation(SBH_DESCRIPTION);
            if(origAnno != null) {
                String origDesc = origAnno.getStringValue();
                String newDesc = origDesc.concat("\n"+description);
                plasmid.getAnnotation(SBH_DESCRIPTION).setStringValue(newDesc);
            } else {
                plasmid.createAnnotation(SBH_DESCRIPTION, description);
            }
        }

        if(notes != null && !notes.isEmpty()) {
            Annotation origAnno = plasmid.getAnnotation(SBH_NOTES);
            if(origAnno != null) {
                String origNotes = origAnno.getStringValue();
                String newNotes = origNotes.concat("\n"+notes);
                plasmid.getAnnotation(SBH_NOTES).setStringValue(newNotes);
            } else {
                plasmid.createAnnotation(SBH_NOTES, notes);
            }
        }
    ...
    protected void checkCompletness(Map<String, String> leftFlanks, Map<String, String> rightFlanks) {
        ...
        if (ONLY_FULL && errors) {
            throw new IllegalArgumentException("Some ids have one of the flank missing");
        }
    }
